...
| `speech/Sounds`        | Short cues played through the sound card (confirmation and failure tones, the four directions of a quick-time event, blips whose pitch follows a ring or a bar, a beacon placed between the ears and pitched by distance), so they reach the player at once, while a held key keeps the reader silent or the reader is busy. |
...
| `input/InputNames`     | Action name → spoken key name for the active control scheme (`APlayerControllerSMGBase::CurrentControlScheme`, `GetKeysFromActionMapping` on the screen on display, else the engine's `InputSettings` action mappings by reflection, `Key_GetDisplayName`, curated names in the language tables); watches keyboard, mouse and gamepad activity so speech knows what the player asked for. |
...
| `features/*`           | One feature per game area, each installing hooks/pollers and contributing to the read-screen (F6) and help (F8) readouts: `Menus` (screens, focus, values, prompts), `Pause` (the pause tab bar), `Hud` (captions, notifications, alerts, loading/saving), `Subtitles` (subtitle lines as they appear), `Prompts` (button prompts with their key), `Choices` (choices, the four-way choice, the countdown, the timer bar), `Qte` (the direction of a quick-time event and its result), `ButtonMash` (the button to mash, the ring, the outcome), `DontBreathe` (the breath prompts, the breath bars, the outcome), `Exploration` (what can be walked to, the beacon, looking around, the reading pane). |
