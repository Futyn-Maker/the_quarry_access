...
| `speech/Speech`        | Policies: `Focus` (interrupting, cascade-debounced), `Announce` (ordered), `Queue` (bounded, subtitles), `Now` (hard interrupt), `Stop`, `Repeat`.                                                                              |
...
| `input/InputNames`     | Action name → spoken key name for the active control scheme (`APlayerControllerSMGBase::CurrentControlScheme`, `GetKeysFromActionMapping`, `Key_GetDisplayName`, curated names in the language tables).                         |
...
| `features/*`           | One feature per game area; each installs hooks/pollers and contributes to the read-screen (F6) and help (F8) readouts.                                                                                                          |
