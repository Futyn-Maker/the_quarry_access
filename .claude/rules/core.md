...
- Every walk has a depth, and a walk that stops short reports nothing about what it did not reach: text below the limit reads as text that is not there. A popup that lists something builds a widget per row, and a row's own words sit a dozen levels under it, which is how the players left without a character went unread and unnoticed by an audit. Raising the limit for everything is not the answer — it drags in the design-time placeholders that sit deeper still — so the screen that keeps its words deep is read where it is described, and the screen dump walks deeper than any reading, since a diagnostic that hides text is worse than none.
...
- Reading a value a hook gave (`params::Int` on the `FFrame`) beats inferring the same thing from what the game does afterwards. A native UFunction that takes a result (`OnQuitToFEPopupEnded(PopupIndexSelected)`) is a delegate target, and `OnNative` fires on it.
